...
- 生成**交互式章节测试 HTML**：选择+判断+填空+简答+计算+代码 9 种题型，点击选项→一键批改→逐题显示正确/错误+详细解析+易错提醒
...
**生成章节知识清单 + 测试题** — 在课程目录下调用 `/exampass`，自动扫描子文件夹、提取内容、深度分析、生成 HTML。
...
**保持更新** — 运行 `/exampass update`，一键拉取最新功能、修复和依赖。
...
| 命令 | 说明 |
|---------|-------------|
| `/exampass` | 生成知识清单和交互式章节测试 |
| `/exampass update` | 一键更新到最新版本（拉取代码+安装依赖） |
| `/exampass-final` | 生成仿真期末考试试卷 + 答案 |
...
├── exampass-update.md           # /exampass-update 入口
